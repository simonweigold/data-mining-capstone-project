...
Before starting with the collection of data, all used packages are loaded now.
```{r}
library(tidyverse)
library(rio)
library(here)
...
The VADER sentiment analysis was conducted using Python (see *scripts/03 - sentiment analysis of news articles.py*). To ensure the functionality of this markdown file, the ouput from the Python code is simply imported as a CSV.
